...
from lottery_core import (
    ...
    ComprehensiveWeightModel          # ⭐ 新增综合权重模型
)
import warnings
...
with tabs[0]:
    if st.session_state.predictions is None:
        ...
        # ⭐ 综合权重模型（改进版核心，优先展示）
        # ================================================================
        comp_preds = preds.get('综合权重', preds.get('融合预测', []))
        comp_debug = preds.get('综合权重调试', {})

        st.markdown("""
        <div style="background: linear-gradient(135deg, #1e3a8a 0%, #7c3aed 50%, #db2777 100%);
                    padding: 1.2rem 1.5rem; border-radius: 12px; margin-bottom: 1rem;">
            <h2 style="color: white; margin: 0; font-size: 1.6rem;">
                ⭐ 综合权重模型（强力推荐）
            </h2>
            <p style="color: rgba(255,255,255,0.85); margin: 0.4rem 0 0 0; font-size: 0.95rem;">
                动态权重 × 温度校准 × 多模型共识 × Transformer 融合
            </p>
        </div>
        """, unsafe_allow_html=True)
        ...
        with col_main:
            st.caption(f"TOP {len(comp_preds)} 综合推荐号码  |  共识票数=被几个独立模型同时选中（最高5）")
            grid_cols = st.columns(2)
            for i, pred in enumerate(comp_preds):
                with grid_cols[i % 2]:
                    conf_color = {'高': '#22c55e', '中': '#f59e0b', '低': '#94a3b8'}[pred['置信度']]
                    votes      = pred.get('共识票数', '-')
                    vote_stars = '⭐' * int(votes) if isinstance(votes, int) else ''
                    st.markdown(f"""
                    <div style="background: linear-gradient(135deg, #1e1b4b 0%, #312e81 100%);
                        ...
                        <span style="font-size:1.4rem; font-weight:bold;">#{i+1}  号码 {pred['号码']}</span>
                        <br>
                        <span style="color:{conf_color}; font-size:0.85rem;">● {pred['置信度']}</span>
                        <span style="color:rgba(255,255,255,0.7); font-size:0.85rem; margin-left:0.8rem;">
                            概率 {pred['概率']}
                        </span>
                        <span style="color:#fbbf24; font-size:0.85rem; margin-left:0.8rem;">
                            共识 {vote_stars} {votes}/5
                        </span>
                    </div>
                    """, unsafe_allow_html=True)
        ...
        with col_side:
            # 模型动态权重可视化
            if comp_debug.get('weights'):
                st.caption("📊 各信号源动态权重（基于近15期命中率）")
                weights   = comp_debug['weights']
                hit_rates = comp_debug.get('hit_rates', {})
                ...
                for k, label in label_map.items():
                    w  = weights.get(k, 0)
                    hr = hit_rates.get(k, 0)
                    st.markdown(
                        f"**{label}**  `{w*100:.1f}%` &nbsp; 近期命中率 `{hr*100:.1f}%`"
                    )
                    st.progress(min(w, 1.0))

                tw = comp_debug.get('transformer_weight', 0)
                if tw > 0:
                    st.markdown(f"**🤖 Transformer** &nbsp; `{tw*100:.0f}%` 混合权重")

            # 辅助预测
            st.divider()
            st.caption("🎲 辅助预测")
            big_small = preds['大小预测']
            color_pred = preds['波色预测']
            color_map = {'红波': '#dc2626', '蓝波': '#2563eb', '绿波': '#16a34a'}
            st.markdown(f"""
            <div style="display:flex; gap:0.5rem; flex-wrap:wrap;">
                <div style="background:{'#fecaca' if big_small=='大' else '#dbeafe'};
                            padding:0.6rem 1.2rem; border-radius:8px; text-align:center; flex:1;">
                    <b style="font-size:1.6rem; color:{'#dc2626' if big_small=='大' else '#2563eb'};">{big_small}</b>
                    <p style="margin:0; font-size:0.75rem; color:#6b7280;">大小</p>
                </div>
                <div style="background:{color_map[color_pred]}22;
                            padding:0.6rem 1.2rem; border-radius:8px; text-align:center; flex:1;">
                    <b style="font-size:1.6rem; color:{color_map[color_pred]};">{color_pred}</b>
                    <p style="margin:0; font-size:0.75rem; color:#6b7280;">波色</p>
                </div>
            </div>
            """, unsafe_allow_html=True)
        ...
        # 调试信息折叠展示
        if comp_debug:
            with st.expander("🔍 综合权重模型调试信息（点击展开）"):
                dc1, dc2 = st.columns(2)
                with dc1:
                    st.write("**各信号源近15期TOP5命中率**")
                    for k, v in comp_debug.get('hit_rates', {}).items():
                        st.metric(label_map.get(k, k), f"{v*100:.1f}%")
                with dc2:
                    st.write("**共识加权最高的5个号码**")
                    for num, bonus in comp_debug.get('consensus_bonus_top5', {}).items():
                        extra = round((bonus - 1) * 100)
                        st.write(f"`{num}` 共识奖励 **+{extra}%**")
        ...
        # 旧版 AI 融合预测（作为对比参考）
        # ================================================================
        with st.expander(f"📊 旧版AI融合预测 TOP {top_k}（等权平均，仅供对比）"):
